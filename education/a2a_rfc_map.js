  ...
  const groups = [
    {
      ...
      items: [
        ...
        item(8259, 'The JavaScript Object Notation (JSON) Data Interchange Format', 'A2AのJSON表現と、入力検証・相互運用性の基準。', 'The JSON representation and interoperability baseline used by A2A.', 'A'),
        item(8615, 'Well-Known Uniform Resource Identifiers (URIs)', '`.well-known/agent-card.json`を評価するための背景。現行A2A 1.0本文の直接引用ではない。', 'Background for `.well-known/agent-card.json`; not a direct citation in current A2A 1.0.', 'B'),
        ...
        item(7515, 'JSON Web Signature (JWS)', 'Agent Card署名とdraftのauthority grant / session proofで使う署名コンテナ。', 'The signature container used for Agent Cards and the draft grant/proof profiles.', 'AD'),
        item(8785, 'JSON Canonicalization Scheme (JCS)', 'Agent Card署名対象を実装間で同じbyte列にする。A2Aのfield presence規則も併読する。', 'Produces stable Agent Card signature bytes; read with A2A field-presence rules.', 'A'),
    ...
    },
    {
      ...
      items: [
        ...
        item(8693, 'OAuth 2.0 Token Exchange', 'actor、subject、audienceを含む委任・token変換。agent chainでのauthority伝搬を議論する基礎。', 'Delegation and token exchange across actors, subjects, and audiences in agent chains.', 'D'),
        ...
        item(9449, 'OAuth 2.0 Demonstrating Proof of Possession (DPoP)', 'HTTP requestごとのproofでtokenをkey-boundにする。認証・認可そのものとの境界が重要。', 'Binds tokens to a key with per-request proofs; not by itself authentication or authorization.', 'D'),
        item(9700, 'Best Current Practice for OAuth 2.0 Security', 'OAuth 2.0を現在の脅威モデルで運用するためのSecurity BCP。', 'The current OAuth 2.0 security BCP and deployment threat model.'),
    ...
    },
    {
      ...
      items: [
        ...
        item(9266, 'Channel Bindings for TLS 1.3', '`tls-exporter` channel binding typeの性質と利用制約。', 'Properties and use constraints of the `tls-exporter` channel-binding type.', 'D'),
        ...
        item(9334, 'Remote ATtestation procedureS (RATS) Architecture', 'Attester、Verifier、Relying Party、Evidence、Attestation Resultの責務分離。', 'Separates Attester, Verifier, Relying Party, Evidence, and Attestation Result roles.', 'D'),
        item(9421, 'HTTP Message Signatures', 'HTTP componentを選んでend-to-endに署名する。cover対象はapplication profileの責務。', 'Signs selected HTTP components end to end; the application profile chooses coverage.', 'D'),

...
- 解説には最低限、以下を含める（学習優先・必須）:
   1. 設問の背景/出題の意図（**問題を出した背景**）
   2. 問われている内容の説明（定義/前提・何を判断する問題か）
   3. 実際での機会（現場でいつ遭遇し、どう役に立つか）
   4. 言葉/用語の説明（必要ならミニ用語集として）
   5. 特に記憶すべきキーワードは **太字** にする
   6. 次に、各選択肢（または各入力パターン）を**丁寧に**解説する
      - 正解が正しい理由だけでなく、**各不正解が間違いである理由**を具体的に説明する
      - 解説を**1行で追えない**こと（短すぎる断定で終わらない）
   7. Topic/関連事項（隣接概念・関連RFC/仕様・落とし穴など）を追加する
